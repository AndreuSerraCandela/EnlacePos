...
codeunit 75200 Importaciones
{
    ...
    begin
        ...
        foreach JToken in JFacturas do begin
            ...
            SalesHeaderT."Tipo Documento SrI" := GetValueOption(GetValueAsText(JToken, 'Tipo_Documento_SrI'), SalesHeaderT);
            SalesHeaderT."VAT Registration No." := GetValueAsText(JToken, 'VAT_Registration_No_');
            ...
            if SalesHeaderT."No. Serie NCF Abonos" <> '' then
                Pedido."No. Serie NCF Abonos" := SalesHeaderT."No. Serie NCF Abonos";
            if SalesHeaderT."Establecimiento Factura" <> '' then
                Pedido."Establecimiento Factura" := SalesHeaderT."Establecimiento Factura";
            if SalesHeaderT."Punto de Emision Factura" <> '' then
                Pedido."Punto de Emision Factura" := SalesHeaderT."Punto de Emision Factura";
            if SalesHeaderT."No. Autorizacion Comprobante" <> '' then
                Pedido."No. Autorizacion Comprobante" := SalesHeaderT."No. Autorizacion Comprobante";
            ...
            Pedido."Tipo Documento SrI" := SalesHeaderT."Tipo Documento SrI";
    ...
    begin
        ...
        foreach JToken in JEmps do begin
            EmpT."Id" := GetValueAsText(JToken, 'No_');
            Deleted := GetValueAsBoolean(JToken, 'Deleted');
            ...
            EmpT.Contrasena := GetValueAsText(JToken, 'Password');
            If (EmpT."Id" = 'TEMP') or (EmpT."Id" = '') Then begin

                HumanResSetup.Get();
                HumanResSetup.TestField("Employee Nos.");
                Emp := EmpT;
                Emp."No. Series" := HumanResSetup."Employee Nos.";
                Emp."Id" := NoSeriesMgt.GetNextNo(HumanResSetup."Employee Nos.", Today, true);
                Emp.Insert();
                EmpT."Id" := Emp."Id";
    ...
    begin
        ...
        foreach JToken in JLFacturas do begin
            ...
            SalesLinet.Descuento := GetValueAsDecimal(JToken, 'Dto_Amount');
            FacturasL := SaleslineT;
            If FacturasL.Insert() Then begin
                FacturasL.Validate(Importe);
                FacturasL.Validate(Descuento);
                FacturasL.Modify();
            end
        end;
        exit('Ok');
    ...
    local procedure GetValueOption(Value: Text; SalesHeader: Record "Sales Header"): Option
    begin
        case Value of
            'Cedula':
                exit(SalesHeader."Tipo Documento SrI"::Cedula);
            'Pasaporte':
                exit(SalesHeader."Tipo Documento SrI"::Pasaporte);
            'RUC':
                exit(SalesHeader."Tipo Documento SrI"::RUC);
    ...
    [EventSubscriber(ObjectType::Table, Database::"Sales Price", OnAfterRenameEvent, '', true, true)]
    local procedure OnAfterRenameSPEvent(var Rec: Record "Sales Price"; var xRec: Record "Sales Price"; RunTrigger: Boolean)
    var
        SalesPrice: Record "Sales Price";
    begin
        SalesPrice.SetCurrentKey("Source Counter2");
    ...
    local procedure OnAfterModifySPEvent(var Rec: Record "Sales Price"; var xRec: Record "Sales Price"; CurrFieldNo: Integer)
    var
        SalesPrice: Record "Sales Price";
    begin
    ...
    [EventSubscriber(ObjectType::Table, Database::"Sales Price", OnAfterValidateEvent, 'Starting Date', true, true)]
    local procedure OnAfterModifySPEvent2(var Rec: Record "Sales Price"; var xRec: Record "Sales Price"; CurrFieldNo: Integer)
    var
        SalesPrice: Record "Sales Price";
    begin
        if CurrFieldNo <> Rec.FieldNo("Starting Date") then exit;
        SalesPrice.SetCurrentKey("Source Counter2");
        if SalesPrice.FindLast() then
            Rec."Source Counter2" := SalesPrice."Source Counter2" + 1;
        Rec.Modify(false);
    ...
    [EventSubscriber(ObjectType::Table, Database::"Sales Price", OnAfterValidateEvent, 'Ending Date', true, true)]
    local procedure OnAfterModifySPEvent3(var Rec: Record "Sales Price"; var xRec: Record "Sales Price"; CurrFieldNo: Integer)
    var
        SalesPrice: Record "Sales Price";
    begin
        if CurrFieldNo <> Rec.FieldNo("Ending Date") then exit;
        SalesPrice.SetCurrentKey("Source Counter2");
        if SalesPrice.FindLast() then
            Rec."Source Counter2" := SalesPrice."Source Counter2" + 1;
        Rec.Modify(false);
    ...
    local procedure OnAfterDeleteContactEvent(var Rec: Record Contact; RunTrigger: Boolean)
    var
        RegistroBorrado: Record "Registro Borrado";
    begin
        if Rec.IsTemporary() then exit;
    ...
    local procedure OnAfterModifyContactEvent(var Rec: Record Contact; var xRec: Record Contact; RunTrigger: Boolean)
    var
        Contact: Record Contact;
    begin
        if not RunTrigger then exit;
    ...
    var
        ...
        Nc: record "Sales Header";
    begin

        //cONTABILIZAR LINEAS DE PAGO
        If SalesInvoiceHeader."No." <> '' then begin
            NC.SetRange("No. Comprobante Fiscal Rel.", SalesHeader."No.");
            if NC.FindFirst() then begin
                NC."Establecimiento Fact. Rel" := SalesInvoiceHeader."Establecimiento Factura";
                NC."Punto de Emision Fact. Rel." := SalesInvoiceHeader."Punto de Emision Factura";
                NC."No. Comprobante Fiscal Rel." := SalesInvoiceHeader."No. Comprobante Fiscal";
            end;
            ...
            if (Importe > 0) and (SalesInvoiceHeader."No." <> '') then begin
                Detallepago.SetRange("Document No.", SalesHeader."No.");
                if Detallepago.Count = 1 Then Begin
                    ...
                    GenJnlLine.Validate("Account No.", PaymentMethod.CuentaPago(PaymentMethod.Code, SalesHeader.Tienda));
